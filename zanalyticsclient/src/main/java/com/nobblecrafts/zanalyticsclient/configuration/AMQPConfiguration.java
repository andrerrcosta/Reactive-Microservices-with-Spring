...
public class AMQPConfiguration {
    ...
    public TopicExchange analyticsReportTopicExchange(@Value("${amqp.exchange.analytics}") final String exchangeName) {
        log.info("Analytics Exchange created: " + exchangeName);
        return ExchangeBuilder.topicExchange(exchangeName).durable(true).build();
    }

    @Bean
    public Queue panelQueue(@Value("${amqp.queue.analyticsclient}") final String queueName) {
        return QueueBuilder.durable(queueName).ttl((int) Duration.ofHours(1).toMillis()).maxLength(25000).build();
    }

    @Bean
    public Binding handleAnalyticsReport(final Queue panelQueue, final TopicExchange exchangeName,
